...
# TODO: get genre_one_hot in with the rest of the input_data somehow
genre_one_hot = build_one_hot(genre_vocab, list(processed_data["Genre"].values))


data_true = processed_data["Global_Sales"]
input_data = processed_data[["Critic_Score", "Critic_Count", "User_Score"]]
...
for fold, (train_indexes, test_indexes) in enumerate(kf.split(input_data)):
    ...
    for epoch in range(epochs):
        ...
        y_pred = model(torch.index_select(genre_one_hot, 0, torch.tensor(train_indexes, dtype=torch.int32)))
        print(y_pred)
        exit()
        #y_pred = torch.reshape(y_pred, (-1, ))

        y_true = torch.tensor(data_true[train_indexes].values, dtype=torch.float32)
    ...
    print("-------- Fold", fold, "--------")
    model.eval()
    y_pred = model(torch.tensor(input_data.iloc[test_indexes].values, dtype=torch.float32))
    y_pred = torch.reshape(y_pred, (-1,))

    y_true = torch.tensor(data_true[test_indexes].values, dtype=torch.float32)
